...
const fieldClass =
  'min-h-10 w-full rounded-lg border border-[#E2E8F0] bg-white px-3 py-2 text-sm'
    ...
    <div className="space-y-6">
      ...
      {canManage && (
        <form
          ...
          className="grid gap-3 rounded-2xl border border-[#E2E8F0] bg-white p-4 text-sm md:grid-cols-3"
        >
      ...
      <div className="space-y-4 md:hidden">
        {rows.map((row) => (
          <article key={row.id} className="space-y-3 rounded-xl border border-[#E2E8F0] bg-white p-4">
            ...
            {canManage && (
              <form action={asFormAction(updatePartner)} className="grid gap-2 border-t border-[#E7ECF3] pt-3">
                <input type="hidden" name="table" value={table} />
        ...
        {rows.length === 0 && (
          <p className="rounded-xl border border-[#E2E8F0] bg-white p-4 text-center text-sm text-gray-500">
            No records found.
          </p>
        )}
      </div>
